#include "WormholeFitSubnetworkRouter.hpp"
...
void WormholeFitSubnetworkRouter::TXProcess()
{
	for (std::int32_t j = 0; j < Relays.size(); j++)
	{
		...
		if (!rel.buffer.IsEmpty())
		{
			...
			if (flit.flit_type == FLIT_TYPE_HEAD)
			{
				...
				if (Relays[out_port].free_slots_neighbor.read() < flit.sequence_length)
...
WormholeFitSubnetworkRouter::WormholeFitSubnetworkRouter(const SimulationTimer& timer, std::int32_t id, std::size_t relays, std::int32_t max_buffer_size) :
